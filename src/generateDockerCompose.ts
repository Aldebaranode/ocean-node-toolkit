...
export function generateDockerCompose(allConfigs: NodeConfig[]) {
  ...
  ${conf.node_name}:
    ...
    environment:
      ...
      P2P_ANNOUNCE_ADDRESSES: '["/dns4/${conf.environment.IP_ADDRESS}/tcp/${conf.ports[0]}/p2p/", "/dns4/${conf.environment.IP_ADDRESS}/ws/tcp/${conf.ports[1]}", "/dns6/${conf.environment.IP_ADDRESS}/tcp/${conf.ports[2]}/p2p/", "/dns6/${conf.environment.IP_ADDRESS}/ws/tcp/${conf.ports[3]}"]'
    networks:
      - ocean_network
    depends_on:
